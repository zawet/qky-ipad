...
/*********一，数据初始化**************/		
	var qkyhead_data={/*导航数据*/
		...
		setup:function(){//常用设置按钮点击事件回调
			$(".setup").on("click",function(){
				$("#comapp_setup").addClass("in").slideDown(500);
			});	
			$(".close,.qkyhead_btn.dist").on("click",function(){
				$("#comapp_setup").removeClass("in").slideUp(500);
			});	
		},
	...
	//已经有的图标名记录数组;
	var haveicon=["changyongshezhi","wangshangwenjuan","xiaoyuanbangong","xiaoyuanzhifu","yidongxiaoyuan","zhihuikongjian","zhihuixiaoyuan","zhinengpaike","zhinengxiaoyuan","zonghepingjia","zonghesuzhifenxi","zonghesuzhipingjia","jichushujufenxi","jiaoshichengchangdangan","xuexiguanlipingtai"] ;
	

	var opts=qkyhead_data;
	var appname=[];var apphref=[];//缓存所有应用名字/应用跳转
	var navmuchhtml=
	 '<li class="qkyhead_select">'+
        '<a class="qkyhead_select_btn clear">	'+
            '<div class="qkyhead_select_value fl">更多</div>'+
            '<div class="qkyhead_sj fl"></div>'+
        '</a>'+
        '<ul class="qkyhead_select_menu" id="navmuchli"></ul>'+
    '</li> ';
        ...
        "qkyhead": function (options) {
            ...
            return this.each(function (i) {
				...
				htmlajax(opts.htmlsrc,function(thishtml){
					...
					thisdiv.addClass("qkyhead").html(thishtml);//给html框架
					
					//1.初始化基础显示数据
					$(".qkyhead_logo img").attr("src",opts.logosrc);
					$(".qkyhead_pjname").html(opts.pjname);
					$(".qkyhead_infoname").html(opts.tea_info.name);
					
					//1.1主导航渲染
					tonavfor($(".qkyhead_nav"),opts.navli_j);
					tonavhtml(opts.navli_j);//超出一半进行折起变更多
					
					//1.2是否显示个人信息
					if(opts.isinfo){
						$(".qkyhead_info").removeClass("yc");
						tofor($("#otherli"),opts.otherli,"lia");
						
						if(opts.tea_info.isphoto){
							$(".qkyhead_infophoto").removeClass("dist").html("<img src='"+opts.tea_info.photo+"' />");
						}else{
							$(".qkyhead_infophoto").addClass("dist");
						}
					}
					...
					if(opts.isnews){
							$(".qkyhead_news").show();
							$(".qkyhead_allnaws_check").attr("href",opts.newscenter_link);//给消息跳页加链接
							if(newsdata.length<=0){
								$(".qkyhead_news_badge").hide();
							} else{
								$(".qkyhead_news_badge").show();
								//获取未读消息并渲染
								$(".qkyhead_news_badge").html(newsdata.length>99 ? 99 : newsdata.length);
								for(var i=0;i<newsdata.length;i++){
									if(i<5){//只显示5条
										var appicon=getpy(newsdata[i].appname);//获取应用名的拼音
										if($.inArray(appicon,haveicon)!=-1){//判断是否有图标了，有的话就加上图标，没有就显示默认app图标
											$("#qkyhead_news_li_mould .app_icon").html('<img src="'+opts.newsimgsrc+appicon+'.png" alt="">');
										}else{
											$("#qkyhead_news_li_mould .app_icon").html("APP");
										}
										$("#qkyhead_news_li_mould .qkyhead_news_name").html(newsdata[i].newsname);
										$("#qkyhead_news_li_mould .qkyhead_news_times").html(newsdata[i].newsgettime);
										$("#qkyhead_news_li_mould .qkyhead_news_appname").html(newsdata[i].appname);
										$("#qkyhead_news_li_mould .qkyhead_news_cont").html(newsdata[i].newscont);
										$(".qkyhead_news_libox").append($("#qkyhead_news_li_mould").html());
									}
									var newspoph=$(".qkyhead_news_popup").outerHeight();
									if(newspoph>500)$(".qkyhead_news_popup").css("height","500px");//超过500的高度自动变可以滚动
								}
								
								$(document).on("click",":not('.qkyhead_news')",function(){
									 $(".qkyhead_news .qkyhead_news_popup").slideUp(50);
								  })
								  $(".qkyhead_news").on("click",function(event){
									 event.stopPropagation();
								  });
								$(".qkyhead_news_icon").click(function(){
									$(this).parent().find(".qkyhead_news_popup").slideToggle(50);
									$(this).find("span").hide();
									$(".qkyhead_more").removeClass("open");
					...
					//1.4是否显示更多弹窗
					if(opts.morebtn){
						$(document).on("click",":not('.qkyhead_more,.qkyhead_menuicon,#comapp_setup')",function(){
							 $(".qkyhead_more").removeClass("open");
							 $(".qkyhead_menuicon").removeClass("active");
						  })
						...
						$(".qkyhead_appCenter").attr("href",opts.appcenter_link);//给应用中心跳页加链接
						if( opts.allapp == "" || opts.allapp == undefined || opts.allapp == null){
							$(".qkyhead_more_havedata").hide();
							$(".qkyhead_more_nodata").show();
						}else{
							//渲染全部app
							var rowi=0;
							for(var key in opts.allapp){
								$("#qkyhead_more_rightOne .allapp_type").html(key).parent().attr("isType",key);
								$("#qkyhead_more_rightTwo .allapp_li").attr("isType",key);
								tofor($("#qkyhead_more_rightTwo .allapp_li"),opts.allapp[key],"a");
								$(".qkyhead_more_right.one .scroll_box").append($("#qkyhead_more_rightOne").html());
								$(".qkyhead_more_right.two .scroll_box").append($("#qkyhead_more_rightTwo").html());
								$("#qkyhead_more_rightTwo .allapp_li").html("");
								rowi++;
						...
						var l=$(".qkyhead_more_left");
						var l_i=$(".qkyhead_more_left .qkyhead_more_lli");
						var l_c=$(".qkyhead_more_left .qkyhead_appCenter");
						var rb=$(".qkyhead_more_rightbox");
						var ro=$(".qkyhead_more_right.one");
						var rt=$(".qkyhead_more_right.two");
						var ro_t=$(".qkyhead_more_right.one .allapp_type_li");
						var rtt=".qkyhead_more_right.two .allapp_li";
						
						$(document).mousemove(function(e){
							var wh= Number($(window).height());	
							var leftw=Number(l.outerWidth()); 
						...
						l_c.hover(function(){ro.addClass("open");},function(){});
						
						ro_t.hover(function(){
							$(this).addClass("active").siblings().removeClass("active");
							rt.addClass("open");
							$(rtt+"[isType='"+$(this).attr("isType")+"']").show().siblings().hide();
						},function(){});
						rb.hover(function(){},function(){
							ro.removeClass("open");rt.removeClass("open");ro_t.removeClass("active");
						});
						rt.hover(function(){},function(){
							rt.removeClass("open");ro_t.removeClass("active");
						});
					...
					$(".qkyhead_nav li").on("click","a",function(){
						opts.navclick($(this));
					});//主导航点击回调
					$(".qkyhead_nav li a").hover(
						function(){
...
//超出一半后折起来
tonavhtml =function(navdata){
		var leftwidth;
		if(opts.morebtn)leftwidth=$(".qkyhead_menuicon").outerWidth(true)+$(".qkyhead_pjname").outerWidth(true)+$(".qkyhead_logo").width()+1;
		else leftwidth=$(".qkyhead_pjname").outerWidth(true)+$(".qkyhead_logo").width()+1;
		if(navigator.userAgent.indexOf("MSIE 8.0")>0) leftwidth=leftwidth+40;
		var havewidth=$(".qkyhead_nav").width()-(leftwidth+$(".qkyhead_other").width());
		havewidth=havewidth/2;

		var temwidth=0;
		var muchli=[];
		var onefor=true;
		$(".qkyhead_nav li").each(function(i) {
            temwidth+=$(this).width();
			if(havewidth<=temwidth){
				muchli.push([navdata[i].name,navdata[i].attrcont,navdata[i].isactive]);
				$(this).hide().remove();	
			}
        });
		if(muchli.length!=0){
			$(".qkyhead_nav").append(navmuchhtml);
